...
const LikeButton = ({
  ...
  useEffect(
    () => setCount(getKnownLikeCount(type, id, likeCount)),
    [likes, type, id, likeCount],
  );
  ...
  return (
    <button
      type='button'
      className={`${styles.button} ${overlay ? styles.overlay : ''} ${liked ? styles.liked : ''} ${isAnimating ? styles.pop : ''}`}
      onClick={handleClick}
      disabled={busy}
      aria-label={liked ? 'いいねを解除' : 'いいね'}
    >
      {liked ? <GoHeartFill /> : <GoHeart />}
      {isLikeCountsLoaded && <span>{count}</span>}
    </button>
  );
};
