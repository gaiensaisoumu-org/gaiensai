...
export const getKnownLikeCount = (
  type: LikeType,
  id: number,
  fallback: number,
      ...
      .then(({ performance_acceptance: data }) => {
        if (Array.isArray(data)) {
          for (const row of data as Array<{
            ...
            if (
              ['class', 'gym', 'club'].includes(row.performance_type) &&
              Number.isInteger(row.performance_id)
            ) {
              likeCounts.set(
                likeKey(row.performance_type, row.performance_id),
                Number(row.like_count) || 0,
              );
              if (row.is_accepting !== null)
                acceptance.set(
                  likeKey(row.performance_type, row.performance_id),
                  row.is_accepting,
                );
...
export const useLikedPerformances = () => {
  ...
  const [isLikeCountsLoaded, setIsLikeCountsLoaded] =
    useState(likeCountsLoaded);
  useEffect(() => {
    const sync = () => {
      setLikes(readLikes());
      setIsLikeCountsLoaded(likeCountsLoaded);
    };
  ...
  const toggle = async (
    ...
    const index = current.findIndex(
      (like) => like.type === type && like.id === id,
    );
    const liked = index >= 0;
    if (!liked && current.length >= 10)
      return { liked: false, likeCount: 0, limitReached: true };
    const { data, error } = await supabase.rpc('change_performance_like', {
      p_type: type,
      p_id: id,
      p_delta: liked ? -1 : 1,
    });
  ...
  return {
    ...
    toggle,
    isLikeCountsLoaded,
    acceptance: isLikeCountsLoaded ? acceptance : null,
  };
};
