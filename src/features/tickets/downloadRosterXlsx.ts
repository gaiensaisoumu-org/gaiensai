...
const addRosterSheet = (
  ...
  const baseSheetName = roster.name.replace(/[\\/?*:]/g, ' ')
    .replaceAll('[', ' ')
    .replaceAll(']', ' ') || '名簿';
  ...
  const headers = ['連番', '学年・クラス・番号', '氏名', '間柄', 'コード番号', '発行日'];
  const rows: string[][] = [
    ['クラス・部活名', '', roster.name],
    ['出力日', new Intl.DateTimeFormat('ja-JP', { year: 'numeric', month: 'numeric', day: 'numeric' }).format(new Date())],
    roster.rounds.flatMap((round) => [round.name, '', '', '', '', '']),
    roster.rounds.flatMap(() => headers),
  ];
  const maxLength = Math.max(roster.generalCapacity, ...ticketsByRound.map((tickets) => tickets.length));
  for (let index = 0; index < maxLength; index += 1) {
    rows.push(ticketsByRound.flatMap((tickets) => {
  ...
  roster.rounds.forEach((_, index) => worksheet.mergeCells(3, index * 6 + 1, 3, index * 6 + 6));
  ...
  for (let column = 1; column <= lastColumn; column += 1) {
    ...
    performanceCell.font = { name: 'Yu Gothic', bold: true, color: { argb: 'FFFFFFFF' } };
    performanceCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F4E78' } };
    ...
    headerCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD9EAF7' } };
  ...
  worksheet.columns = roster.rounds.flatMap(() => [
    { width: 8 }, { width: 20 }, { width: 18 }, { width: 14 }, { width: 16 }, { width: 22 },
  ]);
};
...
export const downloadRosterXlsx = async ({
  ...
  const relationshipNames = new Map(relationships.map(({ id, name }) => [id, name]));
  const usedSheetNames = new Set<string>();
  rosters.forEach((roster) => addRosterSheet(workbook, roster, relationshipNames, usedSheetNames));
