...
import scanStyles from './OrganizationScan.module.css';
...
export type ScanTarget = {
  kind: 'class' | 'gym' | 'rehearsal';
  performanceId: number | null;
  scheduleId: number | null;
};

type TicketlessEntryCountRecord = {
  count: number;
  updatedAt: string;
};

const getTicketlessEntryTargetKey = (target: ScanTarget) =>
...
const OrganizationEntryPage = ({
  ...
  const selectedGymPerformance = ticketMaster?.gymPerformances.find(
    (performance) => performance.id === scanTarget.performanceId,
  );
  const selectedScanSchedule = ticketMaster?.schedules.find(
    (schedule) => schedule.id === scanTarget.scheduleId,
  );
  const selectedScanRehearsal = ticketMaster?.rehearsals.find(
    (rehearsal) =>
      rehearsal.class_id === scanTarget.performanceId &&
      rehearsal.round_id === scanTarget.scheduleId,
  );
  const gymGroups = Array.from(
    new Map(
      (ticketMaster?.gymPerformances ?? []).map((performance) => [
  ...
  const doesTicketMatchScanTarget = (
    ticket: TicketDecodedDisplaySeed,
  ): boolean => {
    if (scanTarget.performanceId === null) {
      return true;
    }

    if (ticket.performanceId !== scanTarget.performanceId) {
      return false;
    }

    const isRehearsalTicket = ticketMaster?.ticketTypes.some(
      (ticketType) =>
        ticketType.id === ticket.ticketTypeId &&
        ticketType.name === 'クラス公演(リハーサル)',
    );
    if (scanTarget.kind === 'rehearsal') {
      return (
        isRehearsalTicket === true &&
        scanTarget.scheduleId !== null &&
        ticket.scheduleId === scanTarget.scheduleId
      );
    }
  ...
  const dismissTicketSyncWarning = useCallback(() => {
    localStorage.setItem(TICKET_STATUS_CACHE_WARNING_DISMISSED_KEY, '1');
  ...
  const readLocalRecords = useCallback((): ScanRecord[] => {
    try {
  ...
  const writeLocalRecords = useCallback((records: ScanRecord[]) => {
    localStorage.setItem(SCAN_RECORDS_STORAGE_KEY, JSON.stringify({ records }));
  }, []);

  const readTicketlessEntryCounts = useCallback((): Record<
    string,
    TicketlessEntryCountRecord
  > => {
    try {
      const raw = localStorage.getItem(TICKETLESS_ENTRY_COUNTS_STORAGE_KEY);
        ...
        Object.entries(parsed).flatMap(([key, value]) => {
          if (
            ...
            !Number.isSafeInteger(
              (value as TicketlessEntryCountRecord).count,
            ) ||
            (value as TicketlessEntryCountRecord).count < 0 ||
            typeof (value as TicketlessEntryCountRecord).updatedAt !== 'string'
          ) {
            return [];
          }
          return [[key, value as TicketlessEntryCountRecord]];
        }),
      );
    } catch {
      return {};
  ...
  const refreshFromLocalStorage = useCallback(() => {
    ...
    setScanRecords(records.slice(0, 5));
    setEntryCount(
      records
        .filter(
          (record) =>
  ...
  const readTicketStatusCache =
    useCallback((): TicketStatusCachePayload | null => {
      try {
        const raw = localStorage.getItem(TICKET_STATUS_CACHE_STORAGE_KEY);
        const parsed = raw
          ? (JSON.parse(raw) as TicketStatusCachePayload)
          : null;
        if (
          !parsed ||
  ...
  useEffect(() => {
    const targetKey = getTicketlessEntryTargetKey(scanTarget);
    setTicketlessEntryCount(readTicketlessEntryCounts()[targetKey]?.count ?? 0);
  }, [scanTarget, readTicketlessEntryCounts]);

  const updateTicketlessEntryCount = (delta: number) => {
    setTicketlessEntryCount((current) => {
      const next = Math.max(0, current + delta);
      const targetKey = getTicketlessEntryTargetKey(scanTarget);
      const counts = readTicketlessEntryCounts();
      counts[targetKey] = { count: next, updatedAt: new Date().toISOString() };
  ...
  useEffect(() => {
    const cache = readTicketStatusCache();
    setTicketCacheSyncedAt(cache?.syncedAt ?? null);
    if (
      (!cache || cache.tickets.length === 0) &&
      localStorage.getItem(TICKET_STATUS_CACHE_WARNING_DISMISSED_KEY) !== '1'
    ) {
      setTicketSyncWarning(
        'チケット状態キャッシュが未取得です。キャンセル済みチケットを判定するには、Supabaseから取得してください。',
      );
    }
  ...
  useEffect(() => {
    try {
      ...
      const parsed = raw ? (JSON.parse(raw) as Partial<ScanTarget>) : null;
      setScanTarget({
        kind:
          parsed?.kind === 'gym'
            ? 'gym'
            : parsed?.kind === 'rehearsal'
              ? 'rehearsal'
              : 'class',
        performanceId:
          typeof parsed?.performanceId === 'number'
            ...
            : null,
        scheduleId:
          typeof parsed?.scheduleId === 'number' ? parsed.scheduleId : null,
      });
    } catch {
      // Keep the default target when local storage is malformed.
    }
  }, []);
  ...
  const updateScanTarget = (next: ScanTarget) => {
    setScanTarget(next);
    localStorage.setItem(SCAN_TARGET_STORAGE_KEY, JSON.stringify(next));
  };
  ...
  useEffect(() => {
    ...
    if (showReentryModal || showMissingSignatureModal || ticketSyncWarning) {
      return () => {
        // noop
      };
    }
  ...
  async function fetchTicketStatus(ticketId: string) {
    const normalizedTicketId = ticketId.replace(/-/g, '').split('.')[0];
    const cachedTicket = readTicketStatusCache()?.tickets.find(
      (ticket) =>
        ticket.code.replace(/-/g, '').split('.')[0] === normalizedTicketId,
    );
    ...
    const previous = readLocalRecords().find(
      (record) =>
        record.ticket_code.replace(/-/g, '').split('.')[0] ===
          normalizedTicketId &&
        (record.result === 'success' || record.result === 'reentry'),
    );
    const lastUsedAt = previous ? new Date(previous.scanned_at) : null;
    return {
      ticketStatus: previous ? 'duplicate' : 'success',
      ticketUsedAt: lastUsedAt?.toLocaleString() ?? null,
      lastUsedAt,
      masterStatus: null,
    };
  }
  ...
  async function saveScanResult(code: string, result: string, count: number) {
    ...
    const savedNextId = Number(
      localStorage.getItem(NEXT_SCAN_RECORD_ID_STORAGE_KEY),
    );
    const nextId =
      Number.isSafeInteger(savedNextId) && savedNextId > 0
        ? Math.max(savedNextId, firstAvailableId)
        : firstAvailableId;
    ...
    writeLocalRecords([saved, ...records]);
    localStorage.setItem(NEXT_SCAN_RECORD_ID_STORAGE_KEY, String(nextId + 1));
    refreshFromLocalStorage();
    return saved.id;
  }
  ...
  const processScannedValue = async (nextScannedValue: string) => {
    ...
    try {
      ...
      if (!doesTicketMatchScanTarget(decoded)) {
        await saveScanResult(nextScannedValue, 'failed', 1);
        setDecodeErrorWithSound({
          title: '公演確認エラー',
          message:
            '設定中の公演クラス・回と異なるチケットです。対象のチケットを確認してください。',
        });
        return;
      }

      const { ticketStatus, ticketUsedAt, lastUsedAt, masterStatus } =
        await fetchTicketStatus(code);
  ...
  const handleSyncTicketsFromSupabase = useCallback(async () => {
    ...
    try {
      const allTickets: Array<{ code: unknown; status: unknown }> = [];
      for (let from = 0; ; from += TICKET_CACHE_PAGE_SIZE) {
        const { data, error } = await supabase
          .from('tickets')
          .select('code, status')
          .order('code', { ascending: true })
          .range(from, from + TICKET_CACHE_PAGE_SIZE - 1);
        if (error) {
          throw error;
        }
        const page = data ?? [];
        allTickets.push(...page);
        if (page.length < TICKET_CACHE_PAGE_SIZE) {
          break;
        }
      }
      const tickets = allTickets
        ...
        .filter((ticket) => ticket.code && ticket.status);
      const syncedAt = new Date().toISOString();
      localStorage.setItem(
        TICKET_STATUS_CACHE_STORAGE_KEY,
        JSON.stringify({
          syncedAt,
          tickets,
      ...
      localStorage.removeItem(TICKET_STATUS_CACHE_WARNING_DISMISSED_KEY);
      setTicketSyncWarning(null);
      setTicketSyncMessage(`${tickets.length}件のチケット状態を保存しました。`);
    } catch {
      setTicketSyncMessage(
        'Supabaseからの取得に失敗しました。ネットワークと認証設定を確認してください。',
      );
    } finally {
  ...
  function updateLocalRecordCount(logId: number | null, count: number) {
    if (logId === null) {
      return;
    }
    writeLocalRecords(
  ...
  const handleEntryCountChange = async (delta: number) => {
    const next = Math.max(1, entryCountValue + delta);
    ...
    if (targetLogId !== null) {
      updateLocalRecordCount(targetLogId, next);
      setScanRecords((prev) =>
        prev.map((record) =>
  ...
  const handleRecordCountChange = async (logId: number, delta: number) => {
    let next = 1;
    setScanRecords((prev) =>
      prev.map((record) => {
        if (record.id !== logId) {
        ...
        next = Math.max(1, (record.count ?? 1) + delta);
        return { ...record, count: next };
      }),
    );
    updateLocalRecordCount(logId, next);
  };
  ...
  const handleDeleteLog = async (logId: number) => {
    ...
    writeLocalRecords(
      readLocalRecords().filter((record) => record.id !== logId),
    );
    refreshFromLocalStorage();
  };
  ...
  return (
    <>
      ...
      <div className={scanStyles.scanTargetDisplay}>
        <span className={scanStyles.scanTargetLabel}>受付中</span>
        <strong>
          {scanTarget.kind === 'gym'
            ? (selectedGymPerformance?.group_name ?? '部活名未選択')
            : (selectedScanPerformance?.class_name ?? '公演クラス未選択')}
        </strong>
        <span>
          {scanTarget.kind === 'gym'
            ? (selectedGymPerformance?.round_name ?? '回未選択')
            : scanTarget.kind === 'rehearsal'
              ? (selectedScanRehearsal?.round_name ?? 'リハーサル未選択')
              : (selectedScanSchedule?.round_name ?? '回未選択')}
        </span>
      </div>
      <div className={styles.pageShell}>
        {mode === 'register' && (
          <h1 className={baseStyles.pageTitle}>各公演入場</h1>
        )}
        ...
        {isPerformanceInProgress && (
          <span className={scanStyles.performanceInProgress}>公演中</span>
        )}

        {mode === 'scan' && (
          <div className={scanStyles.ticketlessCounter}>
            <span>チケット無し入場</span>
            <div className={styles.recordEntryCount}>
              ...
              <button
                ...
                type='button'
                onClick={() => updateTicketlessEntryCount(-1)}
                aria-label='チケット無し入場者を減らす'
              >
                <FaMinus />
              </button>
        ...
        {mode === 'register' && (
          <section>
            <div className={styles.ticketlessCounterRegister}>
              <span>チケット無し入場</span>
              <div className={styles.recordEntryCount}>
                <span className={styles.recordLabel}>人数:</span>
                <button
        ...
        <section className={styles.statsSection}>
          <div className={styles.statCard}>
            ...
            <p className={styles.statValue}>
              {entryCount + ticketlessEntryCount}人
            </p>
          </div>
        </section>
        ...
        <section className={styles.recordsSection}>
          <div className={styles.recordsTitleRow}>
            ...
            <a href='/organization-history'>すべての履歴</a>
          </div>
                  ...
                  {(record.result === 'success' ||
                    ...
                    <div className={styles.recordEntryCount}>
                      ...
                      <button
                        type='button'
                        className={styles.recordCountButton}
                        onClick={() => handleRecordCountChange(record.id, -1)}
                      >
                        <FaMinus />
                      </button>
            ...
            >
              <div className={styles.modalContent}>
                ...
                <div className={styles.serverUrlDisplay}>
                  ...
                  >
                    {isTicketSyncing ? '取得中...' : 'Supabaseから取得'}
                  </button>
                </div>
                {ticketSyncMessage && (
                ...
                {isTicketSyncing && (
                  <LoadingSpinner message='チケット状態を取得しています...' />
                )}
                <h2 className={styles.modalTitle}>スキャン表示</h2>
                    ...
                    value={
                      scanTarget.performanceId === null
                        ...
                        : scanTarget.kind === 'gym'
                          ...
                          : `class:${scanTarget.performanceId}`
                    }
                    onChange={(event) => {
                      const [kind, rawId] =
                      ...
                      if (kind === 'gym') {
                        const performanceId =
                          (ticketMaster?.gymPerformances ?? []).find(
                            (performance) => performance.group_name === rawId,
                          )?.id ?? null;
                        updateScanTarget({
                          kind: 'gym',
                          performanceId,
                      ...
                      const performanceId = Number(rawId);
                      updateScanTarget({
                        kind: 'class',
                        performanceId,
                        scheduleId: scanTarget.scheduleId,
                      });
                    }}
                  >
                    <option value=''>未選択</option>
                    <optgroup label='クラス公演'>
                      {(ticketMaster?.performances ?? []).map((performance) => (
                        <option
                          key={performance.id}
                ...
                <label className={styles.audioSettingRow}>
                  ...
                  <select
                    value={
                      scanTarget.kind === 'gym'
                        ? (scanTarget.performanceId ?? '')
                        : scanTarget.kind === 'rehearsal'
                          ? `rehearsal:${scanTarget.scheduleId ?? ''}`
                          : (scanTarget.scheduleId ?? '')
                    }
                    onChange={(event) => {
                      ...
                      updateScanTarget(
                        scanTarget.kind === 'gym'
                          ...
                          : {
                              ...
                              performanceId: scanTarget.performanceId,
                  ...
                  >
                    ...
                    {scanTarget.kind === 'gym'
                      ...
                      : [
                          ...(ticketMaster?.schedules ?? []).map((schedule) => (
                            <option key={schedule.id} value={schedule.id}>
                              {schedule.round_name}
                            </option>
                          )),
                          ...(ticketMaster?.rehearsals ?? [])
                            .filter(
                              (rehearsal) =>
                                rehearsal.class_id === scanTarget.performanceId,
                            )
