...
ALTER TABLE public.configs
  ADD COLUMN IF NOT EXISTS max_official_rehearsal_tickets_per_user smallint NOT NULL DEFAULT 1,
  ADD CONSTRAINT configs_max_official_rehearsal_tickets_per_user_check
    CHECK (max_official_rehearsal_tickets_per_user BETWEEN 0 AND 100);

CREATE TABLE IF NOT EXISTS public.student_rehearsal_issue_counters (
  user_id uuid NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
  rehearsal_type text NOT NULL CHECK (rehearsal_type IN ('official', 'unofficial')),
  issued_count integer NOT NULL DEFAULT 0 CHECK (issued_count >= 0),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (user_id, rehearsal_type)
);

INSERT INTO public.student_rehearsal_issue_counters (user_id, rehearsal_type, issued_count)
SELECT t.user_id, r.type::text, count(*)::integer
...
WHERE tt.name = 'クラス公演(リハーサル)' AND t.status = 'valid'
GROUP BY t.user_id, r.type
ON CONFLICT (user_id, rehearsal_type)
DO UPDATE SET issued_count = EXCLUDED.issued_count, updated_at = now();

ALTER TABLE public.student_rehearsal_issue_counters ENABLE ROW LEVEL SECURITY;
CREATE POLICY "student_rehearsal_issue_counters_self_read"
  ON public.student_rehearsal_issue_counters FOR SELECT TO authenticated
  USING (user_id = auth.uid());
GRANT SELECT ON public.student_rehearsal_issue_counters TO authenticated;

CREATE OR REPLACE FUNCTION public.sync_student_rehearsal_issue_counter_on_class_ticket_insert()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_user_id uuid; v_type text;
BEGIN
  SELECT t.user_id, r.type INTO v_user_id, v_type
  FROM public.tickets t
  JOIN public.rehearsals r ON r.class_id = NEW.class_id AND r.round_id = NEW.round_id
  JOIN public.ticket_types tt ON tt.id = t.ticket_type
  WHERE t.id = NEW.id AND t.status = 'valid' AND tt.name = 'クラス公演(リハーサル)';
  IF v_user_id IS NOT NULL THEN
    INSERT INTO public.student_rehearsal_issue_counters (user_id, rehearsal_type, issued_count)
    VALUES (v_user_id, v_type, 1)
    ON CONFLICT (user_id, rehearsal_type)
    DO UPDATE SET issued_count = public.student_rehearsal_issue_counters.issued_count + 1, updated_at = now();
  END IF;
  RETURN NEW;
END $$;

CREATE OR REPLACE FUNCTION public.sync_student_rehearsal_issue_counter_on_ticket_status_change()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_user_id uuid; v_type text; v_delta integer := 0;
BEGIN
  ...
  JOIN public.rehearsals r ON r.class_id = ct.class_id AND r.round_id = ct.round_id
  JOIN public.ticket_types tt ON tt.id = coalesce(NEW.ticket_type, OLD.ticket_type)
  WHERE ct.id = coalesce(NEW.id, OLD.id) AND tt.name = 'クラス公演(リハーサル)';
  IF v_type IS NOT NULL THEN
    INSERT INTO public.student_rehearsal_issue_counters (user_id, rehearsal_type, issued_count)
    VALUES (v_user_id, v_type, greatest(v_delta, 0))
    ON CONFLICT (user_id, rehearsal_type)
    DO UPDATE SET issued_count = greatest(public.student_rehearsal_issue_counters.issued_count + v_delta, 0), updated_at = now();
  END IF;
  IF TG_OP = 'DELETE' THEN RETURN OLD; ELSE RETURN NEW; END IF;
END $$;

CREATE TRIGGER sync_student_rehearsal_issue_counter_on_class_ticket_insert
  AFTER INSERT ON public.class_tickets
  FOR EACH ROW EXECUTE FUNCTION public.sync_student_rehearsal_issue_counter_on_class_ticket_insert();
CREATE TRIGGER sync_student_rehearsal_issue_counter_on_ticket_status_change
  AFTER UPDATE OF status ON public.tickets
  FOR EACH ROW EXECUTE FUNCTION public.sync_student_rehearsal_issue_counter_on_ticket_status_change();
CREATE TRIGGER sync_student_rehearsal_issue_counter_on_ticket_delete
