-- The student dashboard needs the user's own performance limits even before a
-- ticket has been issued, so include all performance metadata in its bootstrap.
  ...
  SELECT jsonb_build_object(
    ...
    'config', (
      SELECT jsonb_build_object(
        ...
        'max_tickets_per_other_performance_user', max_tickets_per_other_performance_user,
        'max_tickets_per_other_club_user', max_tickets_per_other_club_user,
    ...
    'class_ticket_count', (
      ...
      JOIN valid_tickets t ON t.id = ct.id
    ),
    'gym_ticket_count', (
      SELECT count(*)
      FROM public.gym_tickets gt
      JOIN valid_tickets t ON t.id = gt.id
    ),
    'tickets', coalesce(
      (SELECT jsonb_agg(to_jsonb(valid_tickets) ORDER BY created_at DESC) FROM valid_tickets),
      '[]'::jsonb
    ),
    'class_performances', coalesce(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            'id', cp.id,
            ...
            'title', cp.title,
            'max_tickets_per_user', cp.max_tickets_per_user
          )
          ORDER BY cp.id
        )
    ...
    'gym_performances', coalesce(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            'id', gp.id,
            'group_name', gp.group_name,
            'round_name', gp.round_name,
            'start_at', gp.start_at,
            'end_at', gp.end_at
          )
          ORDER BY gp.start_at, gp.id
        )
    ...
    'schedules', coalesce(
      (
        SELECT jsonb_agg(jsonb_build_object('id', ps.id, 'start_at', ps.start_at) ORDER BY ps.id)
