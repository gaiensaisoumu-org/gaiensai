-- 公開リハの日時は回名マスタを正とする。個別の公開リハは、その回の
-- start_time / end_time を複写してチケットに保持する。
ALTER TABLE public.rehearsal_round_names
  ADD COLUMN IF NOT EXISTS start_time timestamptz,
  ADD COLUMN IF NOT EXISTS end_time timestamptz,
  ADD CONSTRAINT rehearsal_round_names_time_order_check
...
BEGIN
  SELECT start_time, end_time INTO v_start, v_end FROM public.rehearsal_round_names WHERE name = btrim(coalesce(p_round_name, ''));
...
BEGIN
  ...
  UPDATE public.rehearsals SET round_name = btrim(p_round_name), start_time = v_start, end_time = v_end, capacity = p_capacity WHERE id = p_id RETURNING * INTO v_result;
  RETURN v_result;
END $$;
