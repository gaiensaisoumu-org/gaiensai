-- 管理画面の通常公演の発券状況・ランキングでは、自主リハーサル券を除外する。
...
WITH rehearsal_ticket_type AS (
  ...
  SELECT id, affiliation FROM public.users WHERE role = 'student'
), valid_tickets AS (
  SELECT id, user_id, person_count
  FROM public.tickets
  WHERE status = 'valid'
    AND ticket_type IS DISTINCT FROM (SELECT id FROM rehearsal_ticket_type)
), class_account_rows AS (
  SELECT format('%s-%s', affiliation / 10000, (affiliation / 100) % 100) AS name,
  ...
  SELECT format('%s-%s', sp.affiliation / 10000, (sp.affiliation / 100) % 100) AS name,
  ...
  SELECT car.name, car.account_count,
...
WITH rehearsal_ticket_type AS (
  ...
  SELECT r.name, count(vt.id) AS ticket_count,
         coalesce(sum(vt.person_count), 0) AS visitor_count
  FROM valid_tickets vt
  JOIN public.relationships r ON r.id = vt.relationship
  GROUP BY r.name
), ticket_type_rows AS (
