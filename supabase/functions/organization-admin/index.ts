...
const password = (value: unknown, name: string) => {
  ...
  if (value.length > 256) {
    throw new HttpError(400, `${name} が長すぎます。`);
  }
  return value.trim();
};
...
const text = (value: unknown, name: string, max = 2000) => {
  if (typeof value !== 'string') {
    throw new HttpError(400, `${name} が不正です。`);
  }
...
const requireAdmin = async (client: SupabaseClient, req: Request) => {
  ...
  const { data, error } = await client
    ...
    .select(
      'id, organization_admin_id, expires_at, organization_admins(id, username, password_hash, class_performance_id, gym_performance_id, exhibition_club_id)',
    )
...
const ownPerformance = async (client: SupabaseClient, admin: Admin) => {
  if (admin.class_performance_id) {
    ...
    if (!data) {
      throw new HttpError(404, '担当公演が見つかりません。');
    }
  ...
  if (admin.exhibition_club_id) {
    ...
    if (!data) {
      throw new HttpError(404, '担当展示部活が見つかりません。');
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const client = createClient(
      getEnv('SUPABASE_URL'),
      getEnv('FOR_ADMIN_SUPABASE_SECRET_KEY'),
      {
    ...
    if (action === 'login') {
      ...
      const { data, error } = await client
        ...
        .select(
          'id, username, password_hash, class_performance_id, gym_performance_id, exhibition_club_id',
        )
      ...
      const { error: insertError } = await client
        .from('organization_admin_sessions')
        .insert({
          organization_admin_id: admin.id,
      ...
      return json(
        { authenticated: true, sessionToken: rawToken, expiresAt },
        corsHeaders,
      );
    }
    ...
    if (action === 'verify') {
      try {
        ...
        return json({ authenticated: true }, corsHeaders);
      } catch (error) {
    ...
    if (action === 'getDashboard') {
      ...
      const generalTickets = (
        (ticketLinks ?? []) as Array<{
          id: string;
          round_id?: number;
          tickets: unknown;
        }>
      ).filter((link) => {
        const ticket = link.tickets as unknown as {
          ticket_type?: number;
        } | null;
      ...
      if (own.kind === 'gym') {
        ...
        const limitsByClub =
          config?.gym_ticket_limits_by_club &&
        ...
        const configuredLimit = Number(
          limitsByClub[own.performance.group_name],
        );
        ...
        return json(
          {
            ...
            tickets: generalTickets.map((link) => ({
              ...
              round_name:
                roundNames.get(
                  (link as { performance_id?: number }).performance_id,
                ) ?? '未設定',
            })),
          },
          corsHeaders,
    ...
    if (action === 'updatePerformance') {
      ...
      if (typeof isAccepting !== 'boolean') {
        throw new HttpError(400, '受付状態が不正です。');
      }
      if (own.kind === 'class') {
    ...
    if (action === 'updateTicketSettings') {
      ...
      if (
        !Number.isInteger(juniorCapacity) ||
        typeof juniorCapacity !== 'number' ||
        juniorCapacity < 0 ||
        juniorCapacity > capacity
      ) {
      ...
      if (
        ...
        (!Number.isInteger(maxTicketsPerUser) ||
          typeof maxTicketsPerUser !== 'number' ||
          maxTicketsPerUser < 0 ||
          maxTicketsPerUser > 100)
      ) {
        throw new HttpError(
          400,
          '自クラスの発行可能枚数は0〜100の整数で指定してください。',
        );
      }
      ...
      for (const item of ticketLinks ?? []) {
        const ticket = item.tickets as unknown as {
          person_count?: number;
        } | null;
        ...
        if (Number.isInteger(roundId)) {
          issuedCountsByRound.set(
            roundId,
            (issuedCountsByRound.get(roundId) ?? 0) +
              (ticket?.person_count ?? 0),
          );
        }
      }
      ...
      if (capacity < largestIssuedCount) {
        throw new HttpError(
          400,
          `現在${largestIssuedCount}人分の有効チケットがある公演回があるため、それ未満には変更できません。`,
        );
      }
      const year = await eventYear(client);
      const update =
        own.kind === 'class'
          ? {
              ...
              junior_capacity: juniorCapacity,
    ...
    if (action === 'updateGymTicketLimit') {
      ...
      if (!config) {
        throw new HttpError(500, '発行上限設定が見つかりません。');
      }
      const limitsByClub =
        config.gym_ticket_limits_by_club &&
        typeof config.gym_ticket_limits_by_club === 'object' &&
      ...
      const { error } = await client
        ...
        .update({
          gym_ticket_limits_by_club: {
            ...limitsByClub,
            [own.performance.group_name]: limit,
          },
        })
        .eq('id', config.id);
      if (error) {
    ...
    if (action === 'changePassword') {
      ...
      if (!(await compare(current, admin.password_hash))) {
        throw new HttpError(400, '現在のパスワードが正しくありません。');
      }
      const { error } = await client
        .from('organization_admins')
        .update({
          password_hash: await hash(next, 10),
          updated_at: new Date().toISOString(),
        })
        .eq('id', admin.id);
      if (error) {
